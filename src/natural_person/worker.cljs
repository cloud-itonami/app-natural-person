...
(defn- env->map
  ...
  （中継先だけが例外で、それは route/mcp-router-url を経由して明示的に出す）。"
  [env]
  (if env
...
(defn- proxy-xrpc
  ...
  (let [url (route/mcp-router-url (env->map env))]
    (-> (.json req)
        ...
        (.catch (fn [e]
                  ;; 到達できなかったことを隠さない。移行前はこの経路が
                  ;; 捕まえられておらず、素の 500 {"message":"Internal Error"} に
                  ;; なって原因が応答に残らなかった（README の欠陥 C）。
                  ;; mcp.etzhayyim.com は移行時点で A レコードを返さないので、
                  ;; これは想像上の経路ではなく今日の既定の結末である。
...
(defn fetch-handler [req env _ctx]
  (let [url (js/URL. (.-url req))
    ...
    (case action
      ...
      :health (json {:ok true :app "natural-person" :runtime "cljs"
