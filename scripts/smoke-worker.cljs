...
;; (test/natural_person/route_test.cljc) はソースの判断を固定するが、bundle が
;; 本当に Worker の形で答えるかは言えない —— export の形、shadow の
;; :advanced-optimization、`shadow.resource/inline` で焼いた CSS は、どれも
;; ビルドを通って初めて存在する。
;;
...
(def sentinel
  ...
  つまり検査が構造的に落ちなくなる。app-ongakuka でこれを実測したので印を使う。"
  "SENTINEL-np02-7c1d")
...
(def env #js {"APP_NANOID" "np02priv9"
              "APP_UI_TYPE" sentinel
              "AGENTGATEWAY_MCP_ROUTER_URL" router-url})

(defn- call [h method path]
  (let [req (js/Request. (str "https://natural-person.etzhayyim.com" path) #js {:method method})]
       ...
       (let [h (.-default m)]
         ...
         (-> (js/Promise.all
              #js [(call h "GET" "/") (call h "GET" "/health")
                   ...
                   (call h "GET" "/healthz")])
             (.then
              (fn [[page health bad pre nf mna healthz]]
                ...
                ;; DDS の検査は **2 本**。「dads-table が在る」は落ちない検査で
                ;; ある —— それは view が出力する markup であって、CSS が 1 バイトも
                ;; 入っていないページにも現れる（app-ongakuka 実測: css 込み 74 /
                ;; css 無し 6、0 にならない）。「component を使ったか」と
                ;; 「stylesheet が実際に入ったか」は別の主張なので別の検査にする。
                ...
                ;; 移行前に 404 だったものは 404 のまま。src/app.ts の /healthz は
                ;; deploy されていなかったので、ここで生やすと移行ではなくなる。
                (check! "GET /healthz stays 404 (never deployed)" 404 (:status healthz))
